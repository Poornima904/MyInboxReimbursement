sap.ui.define(['sap/ui/core/mvc/ControllerExtension'], function (ControllerExtension) {
	...
	return ControllerExtension.extend('reimform.ext.controller.OBJ', {
		// this section allows to extend lifecycle hooks or hooks provided by Fiori elements
		override: {
			/**
             * Called when a controller is instantiated and its View controls (if available) are already created.
             * Can be used to modify the View before it is displayed, to bind event handlers and do other one-time initialization.
             * @memberOf reimform.ext.controller.OBJ
             */
			onInit: function () {
			...
			editFlow: {
				...
				onAfterSave: function () {
					...
					// let uploadset = this.base.getView().mAggregations.content[0].mAggregations.sections[2].mAggregations._grid.mAggregations.content[0].mAggregations._grid.mAggregations.content[0].mAggregations.content.mAggregations.items[0];
					// uploadset.setMode("None");
					sap.ui.getCore().byId("reimform::reimbursementheaderObjectPage--fe::CustomSubSection::Attachment--uploadSet").setUploadButtonInvisible(true);
					// sap.ui.getCore().byId("app1::reimbursementheaderObjectPage--fe::CustomSubSection::Frag1--uploadSet").setUploadEnabled(false);
					var items = sap.ui.getCore().byId("reimform::reimbursementheaderObjectPage--fe::CustomSubSection::Attachment--uploadSet").getItems();
					for (let i = 0; i < items.length; i++) {
						items[i].setVisibleRemove(false);
						items[i].setVisibleEdit(false);
				...
				onAfterEdit: function (oBindingContext) {
					...
					sap.ui.getCore().byId("reimform::reimbursementheaderObjectPage--fe::CustomSubSection::Attachment--uploadSet").setUploadButtonInvisible(false);
